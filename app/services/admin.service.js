...
const { Admin, User, Seller, Coupon, Product, Category, Brand, TodayDeal, BannerImage, TrendingProduct, PopularProduct, Color, OrderStatus, CryptoConversion, Payment, Order, OrderTrackingTimeline, ProductSales } = require("../helpers/db");
...
async function getOrders(req) {
    ...
    try {
        ...
        if (param.searching.value && param.searching.value != "") {
            ...
            var productIdArray = [];
            var products = await Product.find({ name: { $regex: '.*' + search_text + '.*' } }).select("id").exec();
...
async function getPayments(req) {
    try {
        ...
        var options = {
            select: 'id orderId userId invoiceNo amountPaid paymentMode paymentAccount isActive createdAt',
            sort: sortOption,
            populate: [{
                path: "orderId",
                model: Order,
